...
use stanza::style::{HAlign, Header, MinWidth, Palette16, Styles, TextFg, MaxWidth};
...
pub fn rough_search(pkgs: Vec<Package>) {
    let mut table = Table::default()
        .with_cols(vec![
            ...
            Col::new(Styles::default().with(MinWidth(12)).with(HAlign::Left)),
            Col::new(Styles::default().with(MinWidth(12)).with(HAlign::Left)),
            Col::new(Styles::default().with(MinWidth(12)).with(MaxWidth(25)).with(HAlign::Left)),
        ])
        .with_row(Row::new(
            Styles::default().with(Header(true)),
            vec![
    ...
    table.push_rows(build_body_rows_rough(pkgs));
    if get_config_value("config", "prefer_md").unwrap() == "true" {
        let renderer = Markdown::default();
        println!("{}", renderer.render(&table));
    } else {
...
fn build_body_rows_rough(pkgs: Vec<Package>) -> Vec<Row> {
    (1..=pkgs.len())
        .map(|row| {
            let mut cells = vec![];
            let repo = pkgs.iter().nth(row - 1).unwrap().repo.replace('"', "");
