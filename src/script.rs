...
impl CompressionTypes {
    fn extract_tar(archive: PathBuf, _destination: PathBuf) -> Result<(), Box<dyn Error>> {
        let _file = File::open(archive.to_str().unwrap())?;
        Ok(())
    }
    fn extract_7z(_archive: PathBuf, _destination: PathBuf) -> Result<(), Box<dyn Error>> {
        Ok(())
    }
...
pub fn read_build_script<T: ToString>(file: T) {
    ...
    for mut command in build {
        ...
        for i in 0..command.len() {
            ...
            if let Some(value) = global_variables
                .keys()
                .find(|&key| command[i].contains(key))
            {
...
pub fn read_run_script<T>(file: T)
where
    T: ToString,
{
    let defined_vars: HashMap<String, String> = HashMap::new();
...
fn exec_command(
    ...
    mut defined_vars: HashMap<String, String>,
    _global_vars: HashMap<String, String>,
) {
    match command[0].as_str() {
        // Match the command and execute it accordingly. TODO make this more secure and maybe async (may cause a race condition or panics)
        "print" => {
            // pretty self explanatory here man
            command.remove(0);
            println!("{:?}", command);
        }
        "bash" => {
            // Run a bash command in the current dir
            command.remove(0);
        ...
        "make" => {
            // Executes CMake/Make in the specified directory.
            let path = command[1].as_str();
            syscmd("make", vec![path]);
        }
        "clone" => {
            // Clones a git repository. Example: "clone vim/vim $PKGDIR/"
            let repo = format!("https://github.com/{}.git", command[1]);
            println!("Cloning repo {repo}");
            syscmd("git", vec!["clone", &repo]);
        }
        "extract" => {
            // Extract an archive. Format: ARCHIVE/DESTINATION/COMPRESSION TYPE
            match command[3].as_str() {
                "tar" => {
                    CompressionTypes::extract_tar(
                        command[1].clone().into(),
                        command[2].clone().into(),
                    );
                }
                "zip" => {
                    CompressionTypes::extract_zip(
                        Path::new(&command[1].clone()),
                        Path::new(&command[2].clone()),
                    );
                }
                "7z                                              " => {
                    CompressionTypes::extract_7z(
                        command[1].clone().into(),
                        command[2].clone().into(),
                    );
                }
        ...
        "load" => {
            // Load a different build script. Circular loops are possible
